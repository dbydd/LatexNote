...
\subsection{曲线积分与曲面积分}{
  ...
  \subsubsection{第一类曲线积分(对弧长的曲线积分)}{
    ...
    \begin{itemize}
      ...
      \item $\pathIntegral{L}\defFunction{x,y}ds = \pathIntegral{L_1}\defFunction{x,y}ds + \pathIntegral{L_2}g(x,y)ds \qquad (L = L_1 + L_2)$
    ...
    \subsubsection{格林公式的计算}{
      ...
      \begin{enumerate}
        \item {
              ...
              使用格林公式,可以用线积分计算区域的面积.因为区域$D$的面积等于$A = \doubleIntegralOnZone{D}dA$,所以只要选取适当的$P$与$Q$使得$\partialDerivativeFrac{Q}{x} - \partialDerivativeFrac{P}{y} = 1$,就可以通过$A = \curveIntegralOnLine{L}(Pdx + Qdy)$来计算面积.
              ...
              \begin{enumerate}
                ...
                \item {
                      ...
                      \begin{enumerate}
                        ...
                        \item{
                              包括原点,去适当小的$r > 0$,在原点附近作圆,圆周$C: x^2 + y^2 = r^2$,称为复联通区域,方向逆时针.
  ...
  \subsection{雅可比矩阵与雅可比行列式}{
    ...
    \subsubsection{雅可比矩阵}{
      ...
      讲更详细点也就是 :
      $$
        \defFunction{x} = \defFunction{p} + J_f(p)(x - p) + \smallO{\absoluteValue{x - p}}
      $$
  ...
  \subsubsection{正项级数}{
    ...
    \begin{itemize}
      \item {
            ...
            如果$\myLimToInf\cfrac{u_n}{v_n} = 1$或其他有限数,则$\sum u_n$和$\sum v_n$同敛散.
