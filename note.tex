...
\section{场论}{
  ...
  \subsection{场论基本内容}{
    ...
    \subsubsection{矢量函数的导数}{
      ...
      当$\Delta t \to 0$时,$\frac{\Delta A}{\Delta t} = \frac{A(t + \Delta t) - A(t)}{\Delta t}$的极限存在,则称此极限值为$A(t)$在点t处的导向量/导矢。即 :

      \begin{center}
        $A\derivative(t) = \frac{dA(t)}{dt} = \limNormal{\Delta t \to 0}\frac{\Delta A}{\Delta t} = \limNormal{\Delta t \to 0}\frac{A(t + \Delta t) - A(t)}{\Delta t}$
      \end{center}

      以下为导数公式 :

      \begin{itemize}
        \item $\frac{d}{dt}(\vec{\mathConstant}) = 0$ ($\vec{\mathConstant}$为常矢量)
        \item $\frac{d}{dt}(\vec{A} \pm \vec{B}) = \frac{d}{dt}\vec{A} \pm \frac{d}{dt}\vec{B}$
        \item $\frac{d}{dt}(u\vec{A}) = \frac{du}{dt}\vec{A} + u\frac{d\vec{A}}{dt}$\ 当u是常数k时,$\frac{d}{dt}(kA) = k\frac{d\vec{A}}{dt}$
        \item $\frac{d}{dt}(\vec{A}\cdot\vec{B}) = \vec{A}\cdot\frac{d\vec{B}}{dt} + \vec{B}\cdot\frac{d\vec{A}}{dt}$
        \item $\frac{d}{dt}(\vec{A}\times\vec{B}) = \vec{A}\times\frac{d\vec{B}}{dt} + \vec{B}\times\frac{d\vec{A}}{dt}$
        \item 若$\vec{A} = A(u),u = u(t)$,则$\frac{d\vec{A}}{dt} = \frac{d\vec{A}}{du}\cdot\frac{du}{dt}$
      \end{itemize}
    ...
    \subsubsection{矢量函数的微分}{
      ...
      $\frac{d\vec{A}}{dt} = A\derivative_x(t)\vec{i} + A\derivative_y(t)\vec{j} + A\derivative_z(t)\vec{k}$
      ...
      \subsubsection{三维变换}{
        ...
        \begin{itemize}
          ...
          \item 三维旋转($\vec{I}$绕任意轴n旋转角度$\alpha$)(罗德里格斯旋转公式) : $$
                  R(n,\alpha) = \cos(\alpha)\vec{I} + (1 - \cos(\alpha))\vec{n}\vec{n}\transpose + \sin(\alpha)\left[\begin{matrix}
                      0    & -n_z & n_y  \\
                      n_z  & 0    & -n_x \\
                      -n_y & n_x  & 0
                    \end{matrix}\right]
