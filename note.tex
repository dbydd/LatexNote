...
\newcommand{\mathIntegerCollection}{\mathbb{Z}}
\newcommand{\mathNatureNumberCollection}{\mathbb{N}}
...
\newcommand{\mathBigCase}[1]{\left[#1\right]}
\newcommand{\pathIntegral}[1]{\int_{#1}}
\newcommand{\curveIntegralOnLine}[1]{\oint_{#1}}
\newcommand{\doubleCurveIntegralOnLine}[1]{\oiint_{#1}}
...
\pagestyle{empty}{
  ...
  \paragraph{todos}{
    \begin{enumerate}
      \item 誊录纸质笔记\ 线性代数-矩阵乘法,lu分解
      \item 隐函数存在定理,等幂求和,二项式系数,朱世杰恒等式.
      \item 重写线性代数
      \item 场论:p10
...
\subsubsection{极坐标系下的面积}{
  ...
  \subsubsection{球面坐标}{
    ...
    \begin{itemize}
      ...
      \item $\theta$ : 用过原点以$z$轴作为法向量的平面作为极坐标的平面,$\vec{v}$投影到此平面上时$\theta$就是该投影在极坐标中的角度单位.取值范围为$\mathBigCase{0,2\pi}$
      \item $\varphi$ : 以原点为顶点,$z$轴为旋转轴,$\vec{v}$作为母线的圆锥面的半顶角.也就是$\vec{v}$与$z$轴的夹角.取值范围为:$\mathBigCase{0,\pi}$
    \end{itemize}
...
\subsection{正三角函数}{
  \begin{tabular}{|c|c|c|c|}
    ...
    名字           & 定义           & 定义域                                                                                       & 值域                                \\
    \hline
    $\sin{\alpha}$ & $\cfrac{y}{r}$ & $\mathRealNumberCollection$                                                                  & $\mathBigCase{-1,1}$                \\
    \hline
    $\cos{\alpha}$ & $\cfrac{x}{r}$ & $\mathRealNumberCollection$                                                                  & $\mathBigCase{-1,1}$                \\
    \hline
...
\subsection{反三角函数}{
  \begin{tabular}{|c|c|c|c|}
    ...
    名字        & 定义         & 定义域                                                & 值域                                                                 \\
    \hline
    $\arcsin x$ & $x = \sin y$ & $\mathBigCase{-1,1}$                                  & $\mathBigCase{-\cfrac{\pi}{2},\cfrac{\pi}{2}}$                       \\
    \hline
    $\arccos x$ & $x = \sin y$ & $\mathBigCase{-1,1}$                                  & $\mathBigCase{0,\pi}$                                                \\
    \hline
    $\arctan x$ & $x = \tan y$ & $\mathRealNumberCollection$                           & $\mathBigCase{-\cfrac{\pi}{2},\cfrac{\pi}{2}}$                       \\
    \hline
    $\arccot x$ & $x = \cot y$ & $\mathRealNumberCollection$                           & $\mathBigCase{0,\pi}$                                                \\
    \hline
...
\subsection{数列极限相关}{
  ...
  \subsubsection{O'Stolz(stolz)定理}{
    设$(a_n)_{n > 1}$和$(b_n)_{n > 1}$为两个实数数列.若$b_n$为从某项开始严格单调的无界正数数列,且有穷穷极限
    $$
...
\subsubsection{达布中值定理}{
  设$\defFunction{x}$在$(A,B)$区间中可导,且$\mathBigCase{a,b} \in (A,B)$,$\fDerivative{a} < \fDerivative{b}$,则对于任意给定的$\eta : \fDerivative{a} < \eta < \fDerivative{b}$,都存在一点$c \in (a,b)$使得$\fDerivative{c} = \eta$

  即 : 设$f : \mathBigCase{a,b} \to \mathRealNumberCollection$为一个$\mathBigCase{a,b}$上的实值可导函数,并在$\mathBigCase{a,b}$上可导,那么$f\derivative$满足 : 对任意介于$\fDerivative{a}$和$\fDerivative{b}$之间的$t$,存在$x \in (a,b)$使得$\fDerivative{x} = t$.

  等价于 : 设$\defFunction{x}$在$\mathBigCase{a,b}$上可微,若在$\mathBigCase{a,b}$上$\fDerivative{x}$不等于0,则$\fDerivative{x}$在$\mathBigCase{a,b}$上保持定号(恒正或恒负).
}%达布中值定理结尾
...
他的几何意义很明显为:能找到$\xi\in[a,b]$,使得红色区域的面���和蓝色区域的面积相加等于阴影区域的面积,即S[\uppercase\expandafter{\romannumeral1}] = S[\uppercase\expandafter{\romannumeral2}]
...
则$z = \defFunction{x,\Phi(x)},\cfrac{dz}{dx} = \partialDerivativeFrac{f}{x} + \partialDerivativeFrac{f}{y}\cfrac{dy}{dx}$

$\left.\cfrac{dz}{dx}\right|_{x = x_0} = f\derivative_x(x_0,y_0) + f\derivative_y(x_0,y_0)\left.\cfrac{dy}{dx}\right|_{x = x_0} = 0$

% $f\derivative_x(x_0,y_0) - f\derivative_y(x_0,y_0)\cfrac{\varphi\derivative_x}{}$// #TODO 检查笔记有没有写错,顺便学一下拉格朗日乘数法.
}%条件极值与拉格朗日乘数法结尾

\subsubsection{拉格朗日乘数法}{
  引用上一题,作辅助函数$L(x,y) = \defFunction{x,y} + \lambda\varphi(x,y)$
...
\subsection{曲线积分与曲面积分}{
  ...
  \subsubsection{第二类曲线积分(对坐标的曲线积分)}{
    ...
    \subsubsection{格林公式的计算}{
      $$
        \doubleIntegralOnZone{D}\mathBigCase{\partialDerivativeFrac{Q}{x} - \partialDerivativeFrac{P}{y}}dxdy = \curveIntegralOnLine{L}Pdx + Qdy
      $$
  ...
  \subsubsection{正项级数}{
    ...
    \begin{itemize}
      \item {
            ...
            比较判别法的特点是要已知若干级数的敛散性.一般来说,我们可以选择比较简单的级数:$P$级数$U_p = \sum\cfrac{1}{n^p}$作为"标准级数",依此判断其他函数的敛散性.需要知道的是当$p \leq 1$时,$P$级数发散.当$p \geq 1$时,$P$级数收敛.
      ...
      \item {
            ...
            若函数$\defFunction{x}$是区间$\mathBigCase{a,+\infty}$上非负且单调递减的连续函数,则$\upDownSum{\infty}{n = a} a_n = \upDownSum{\infty}{n = a} f(n)$与$\definiteIntegral{a}{+\infty}\defFunction{x}dx$的敛散性一致.
...
\subsubsection{阿贝尔定理}{
  ...
  用人话来说就是 : $\upDownSum{+\infty}{n = 0}a_nx^n$,如果$x = x_0$时此幂级数收敛,如果$\absoluteValue{x} \leq \absoluteValue{x_0}$,则此幂级数绝对收敛.反之,如果$x = x_0$时发散,且$\absoluteValue{x} > \absoluteValue{x_0}$,则幂级数发散.
...
\subsubsection{幂级数的运算}{
  \begin{itemize}
    \item {
          幂级数的加法与减法 : 将相应系数进行加减.
          $$
            (a_0+ a_1x + a_2x^2 + \dots + a_nx^n + \dots) \pm (b_0 + b_1x + b_2x^2 + \dots + b_nx^n + \dots) = (a_0 + b_0) + (a_1 + b_1)x + (a_2 + b_2)x^2 + \dots + (a_n + b_n)x^n + \dots
          $$
          }
    \item {
          幂级数的乘积 : 基于柯西乘积
          ...
          \begin{math}
            $$\bigCase{\upDownSum{\infty}{n = 0}a_n(x - c)^n}\mathBigCase{\upDownSum{\infty}{n = 0}b_n(x - c)^n}$$ \\
...
\subsection{一些杂七杂八的前置/补充}{
  ...
  \subsubsection{关于线性组合}{
    线性组合(Linear combination)是线性代数中的概念,通常的表现形式如下 :
    $$
      \vec{b} = \mathConstant_1\vec{a}_1 + \mathConstant_2\vec{a}_2 + \mathConstant_3\vec{a}_3 + \dots + \mathConstant_n\vec{a}_n
    $$
  ...
  \subsubsection{矩阵的乘法}{
    矩阵的乘法就目前看来有四种看待方法,设$A$与$B$相乘得到$C$,其中$A$为$m \times n$矩阵,而$B$为$n \times p$矩阵,则$C$为$m \times p$矩阵.记$c_{ij}$为矩阵$c$中第$i$行$j$列的元素 :
...
\subsection{$A\vec{x} = \vec{b}$与四个子空间}{
