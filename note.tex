...
\newcommand{\mathZone}[1]{\left[#1\right]}
...
\subsubsection{极坐标系下的面积}{
  ...
  \subsubsection{球面坐标}{
    ...
    \begin{itemize}
      ...
      \item $\theta$ : 用过原点以$z$轴作为法向量的平面作为极坐标的平面,$\vec{v}$投影到此平面上时$\theta$就是该投影在极坐标中的角度单位.取值范围为$\mathZone{0,2\pi}$
      \item $\varphi$ : 以原点为顶点,$z$轴为旋转轴,$\vec{v}$作为母线的圆锥面的半顶角.也就是$\vec{v}$与$z$轴的夹角.取值范围为:$\mathZone{0,\pi}$
    \end{itemize}
...
\subsection{正三角函数}{
  \begin{tabular}{|c|c|c|c|}
    ...
    名字           & 定义           & 定义域                                                                                       & 值域                                \\
    \hline
    $\sin{\alpha}$ & $\cfrac{y}{r}$ & $\mathRealNumberCollection$                                                                  & $\mathZone{-1,1}$                   \\
    \hline
    $\cos{\alpha}$ & $\cfrac{x}{r}$ & $\mathRealNumberCollection$                                                                  & $\mathZone{-1,1}$                   \\
    \hline
...
\subsection{反三角函数}{
  \begin{tabular}{|c|c|c|c|}
    ...
    名字        & 定义         & 定义域                                                & 值域                                                                 \\
    \hline
    $\arcsin x$ & $x = \sin y$ & $\mathZone{-1,1}$                                     & $\mathZone{-\cfrac{\pi}{2},\cfrac{\pi}{2}}$                          \\
    \hline
    $\arccos x$ & $x = \sin y$ & $\mathZone{-1,1}$                                     & $\mathZone{0,\pi}$                                                   \\
    \hline
    $\arctan x$ & $x = \tan y$ & $\mathRealNumberCollection$                           & $\mathZone{-\cfrac{\pi}{2},\cfrac{\pi}{2}}$                          \\
    \hline
    $\arccot x$ & $x = \cot y$ & $\mathRealNumberCollection$                           & $\mathZone{0,\pi}$                                                   \\
    \hline
...
\subsubsection{拉格朗日乘数法}{
  ...
  $u = \defFunction{x,y,z,t},\varphi(x,y,z,t) = 0,\phi(x,y,z,t) = 0$

  $L = \defFunction{x,y,z,t} + \lambda\varphi(x,y,z,t) + \mu\phi(x,y,z,t)$
  ...
  \subsubsection{第一类曲线积分的计算}{
    ...
    \begin{center}
      $$
        ...
        \end{cases}
      $$
      这两个参数方程都有一阶连续导数,且$\left[\varphi\derivative(t)\right]^2 + \left[\phi\derivative(t)\right]^2 \neq 0$.
    \end{center}

    则$\pathIntegral{L}\defFunction{x,y}ds = \definiteIntegral{\beta}{\alpha}\defFunction{\varphi(t),\phi(t)}\sqrt{(\varphi\derivative(t))^2 + (\phi\derivative(t))^2}dt\qquad (\alpha < \beta)$
    ...
    \begin{enumerate}
      ...
      \item{
            $x = \varphi(t),y = \phi(t),z = \omega(t)$

            $\vdots$

            $\definiteIntegral{\beta}{\alpha}\defFunction{\varphi(t),\phi(t),\omega(t)}\sqrt{(\varphi\derivative(t))^2 + (\phi\derivative(t))^2 + (\omega\derivative(t))^2}dt\qquad (\alpha < \beta),(\alpha \leq t \leq \beta)$
            }
    \end{enumerate}
  }%第一类曲线积分的计算结尾
          ...
          \item {
                ...
                \begin{itemize}
                  ...
                  \item 将$\hat{\vec{g}}$旋转到-z方向,将$\hat{\vec{t}}$旋转到y方向,($\hat{\vec{g}} \times \hat{\vec{t}}$)旋转到x方向 : $$
                          ...
                          \end{bmatrix}
                        $$
                  \item 仔细思考,发现上面想要直接计算出来过于繁琐,应当反向思考—求出将坐标系变换到���机的位置的矩阵再求逆 : $$
          ...
          \item {
                ...
                \begin{itemize}
                  ...
                  \item {
                        ...
                        首先把视锥压缩成长方体.(近平面永远不变,远��平面z值不发生变化,中心点不发生变化)$(n \to n,f \to f,M_{persp \to ortho})$
